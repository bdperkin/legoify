...
system(
"inkscape $file --export-background=#FFFFFF --export-png=$file.png -w$widthpx --export-area-drawing"
);
system("convert -remap $colormapgif $file.png $file.tmp.png");

my @WIDTH =
`identify -verbose $file.png | grep png:IHDR.width,height | cut -d: -f3 | cut -d, -f1 | awk '{print \$1}'`;
my $wtest = $WIDTH[0];
chomp $wtest;
if ( $wtest != $widthpx ) {
...
my @HEIGHT =
`identify -verbose $file.png | grep png:IHDR.width,height | cut -d: -f3 | cut -d, -f2 | awk '{print \$1}'`;
my $heightpx = $HEIGHT[0];
chomp $heightpx;

my $heightremainder = $heightpx % 32;
my $heightaddition  = 32 - $heightremainder;
my $newheightpx     = $heightpx + $heightaddition;

system( "convert  -gravity center -extent "
      . $widthpx . "x"
      . $newheightpx
      . " $file.tmp.png $file.png " );
...
while ( $xoffset < ( $widthpx - 1 ) ) {
    while ( $yoffset < ( $heightpx - 1 ) ) {
        ...
        my @COLOR =
`convert $file.px -scale 1x1\! -remap $colormapgif -format '%[pixel:s]' info:-`;
        ...
        if (   $colormap{$srgbcolor} =~ m/BLACK/
            || $colormap{$srgbcolor} =~ m/DARK/
            || $colormap{$srgbcolor} =~ m/DK\./ )
        {
            $stroke = "white";
        } ## end if ( $colormap{$srgbcolor...})
        ...
        if ( $colormap{$srgbcolor} =~ m/CLEAR/ ) {
            $line =
"-draw \"line $xoffset,$yoffset $xoffsetopp,$yoffsetopp\" -draw \"line $xoffset,$yoffsetopp $xoffsetopp,$yoffset\" ";
        }

        system(
"convert $file.lego.png -fill \"$srgbcolor\" -stroke $stroke -strokewidth 1 -draw \"rectangle $xoffset,$yoffset $xoffsetopp,$yoffsetopp\" $line $file.lego.tmp.png"
        );
        system("mv $file.lego.tmp.png $file.lego.png");
        $yoffset = $yoffset + 32;
    } ## end while ( $yoffset < ( $heightpx...))
    $yoffset = 0;
    $xoffset = $xoffset + 80;
} ## end while ( $xoffset < ( $widthpx...))
